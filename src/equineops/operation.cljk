(ns equineops.operation
  "OperationActor -- one equine-facility operation = one supervised actor
  run. The advisor (EquineOpsAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Equine Facility
  ...
  One run = one equine-facility coordination operation (intake -> advise
  -> govern -> decide -> commit | hold | approval). No unbounded inner
  loop -- each operation is auditable and checkpointed. A stable's
  ...
  hands the decision to a human operator (stable operator / veterinarian).
  ...
  Governor is clean -- see `equineops.governor/always-escalate-ops`.
  ...
  (:require [equineops.advisor :as advisor]
...
(defn run-operation
  "Run one equine-facility operation through the advisor -> governor ->
...
(defn build
  ...
  run-operation for testing. opts:
    :advisor -- an `equineops.advisor/Advisor` (default: mock-advisor)"
  [store & [opts]]
